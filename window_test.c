...
/*   Updated: 2024/06/15 21:57:35 by martalop         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main()
{
	...
	cyan_color = 0x0000ffff;
	yellow_color = 0x00FFFF00;
	mlx_ptr = mlx_init();
	...
	c.real = 1.9;
	c.im = 0.5;
	mlx_pixel_put(mlx_ptr, window, 750, 500, 0x0000FF00);
	while (c.real > -2) //voy desde el 0.5 al -2
	{
		count = 0;
		z.real = 0;
		z.im = 0;
		...
		if ((formula_res.real > 2 || formula_res.real < -2) || (formula_res.im > 2 || formula_res.im < -2))
		{
